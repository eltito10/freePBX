...
function fill_cdrcost() {
	...
	while ($next <> '') {
		...
		if (count($costs) > 0) {
			...
			sql($sql,"query");
			$sql_rateid = "SELECT name FROM call_rate WHERE id = '".$rid."'";
	                $result = sql($sql_rateid,"getAll");
                        $rname = $result[0][0];
			$sql2 = "UPDATE asteriskcdrdb.cdr SET userfield = '".$cost."', accountcode = '".$rname."' WHERE uniqueid = '".$next."'";
			sql($sql2,"query");
		}
		$last = $next;
	...
	if ($last <> '') {
                $sql = "UPDATE asterisk.globals SET value = '".$last."' WHERE variable = 'CDRCOST_MAX'";
                sql($sql,"query");
	}
}
